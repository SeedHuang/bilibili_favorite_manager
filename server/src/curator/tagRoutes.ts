...
/**
 * 手动质检的实时状态 —— 照 currentRun 的样子,**轮询 `/api/tags/check-progress` 读**。
 * 手动全库质检是最长几分钟的 LLM 调用,启动即返回 + 前端轮询(和标注同一套 UX),
 * 进度/判定/理由全挂在 logs 上,日志抽屉里看得见。
 */
const currentCheck: {
  running: boolean;
  scope: 'all' | 'new' | null;
...
export function registerTagRoutes(app: FastifyInstance, deps: TagDeps): void {
  ...
  // ── 手动质检的进度查询 —— 和 run-progress 同一套轮询模式 ──
  app.get('/api/tags/check-progress', async () => {
    const { running, scope, done, total, result, error, logs } = currentCheck;
    return { running, scope, done, total, result, error, logs };
  });
  ...
  app.post('/api/tags/tagcheck', async (req, reply) => {
    ...
    currentCheck.scope = scope;
    currentCheck.done = 0;
    currentCheck.total = 0;
    currentCheck.result = null;
    currentCheck.error = null;
    currentCheck.logs = [];
    currentCheck.controller = controller;
    reply.send({ ok: true });

    const t0 = Date.now();
    /** 日志出口 —— 照 run 路由的 frame:写进 currentCheck.logs 让轮询读到 */
    const frame = (event: 'phase' | 'verdict' | 'note', data: unknown) => {
      currentCheck.logs.push({ type: event, ...(data as object) });
    };
    try {
