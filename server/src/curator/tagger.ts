...
export async function runTagging(opts: {
  ...
  for (let i = 0; i < opts.items.length; i += size) {
    ...
    if (pending.size > 0 && !opts.signal?.aborted) {
      ...
      //
      // 「下次增量会再试」只对**真失败**成立 —— 它没写水位线,下一轮还在池子里,
      // 再试一次是它应得的。**已失效条目不能进这个循环**:路由在组池时就把 invalid=1
      // 排掉了(没标题没简介,模型对着占位符什么都吐不出来),否则它会像这里的真失败
      // 一样永远留在 `ai_checked_at IS NULL` 里、每轮都失败 —— 那不是"再试一次"能
      // 救回来的,是池子被它永久堵住(用户报的「0 条,3 批失败」就是 39 条失效视频
      // 占满了 3 批的批上限)。
